...
export default function ViewProduct({ slug, className, product }: IndexProps) {
  ...
  function validateColors(colors: any) {
    ...
    for (const color of colors) {
      if (!color.name || !color.code) {
        toast('Each color must have both a name and a code.');
        setLoading(false);
        return false;
      }
    }
  ...
  const onSubmit: SubmitHandler<CreateProductInput> = (data) => {
    ...
    if (!instagramId) {
      setLoading(false);
      return toast('Please pass a valid Instagram Post Link');
    }
    ...
    if (!data?.tags?.length) {
      setLoading(false);
      return toast('Minimum 1 tag is required');
    }

    if (!data?.sizes?.length) {
      setLoading(false);
      return toast('Minimum 1 size is required');
    }

    if (!data?.images) {
      setLoading(false);
      return toast('Atleast one image is required');
    }
